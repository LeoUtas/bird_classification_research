...
# _______ MOBILENET test19: RE-TRAIN THE WHOLE LAYERS _______ #
start_time = time()
...
# # assign the last 45 layers to be trainable
# last_55_layers = model_base.layers[-55:]
# for layer in last_55_layers:
...
callbacks = [
    ...
    tf.keras.callbacks.ModelCheckpoint(
        "save_best_models/best_model_mobilenet_test19.h5",  # path to save the best model
...
mobilenet_test19_train_accuracy = history.history["accuracy"]
mobilenet_test19_val_accuracy = history.history["val_accuracy"]
mobilenet_test19_train_loss = history.history["loss"]
mobilenet_test19_val_loss = history.history["val_loss"]

# load the saved best model
best_model_0 = tf.keras.models.load_model(
    "save_best_models/best_model_mobilenet_test19.h5"
)

# evaluate the test data using the loaded best model
mobilenet_test19_test_loss, mobilenet_test19_test_accuracy = best_model_0.evaluate(
    test_data
)

end_time = time()
execution_time_test19 = end_time - start_time

logging.info("mobilenet test19 was done")
# --------------------------------------------------------------------------------------- #


# _______ MOBILENET TEST20: RE-TRAIN THE WHOLE MODEL & AUGMENTATION _______ #
...
# # assign the last 55 layers to be trainable
# last_55_layers = model_base.layers[-55:]
# for layer in last_55_layers:
#     layer.trainable = True
...
callbacks = [
    ...
    tf.keras.callbacks.ModelCheckpoint(
        "save_best_models/best_model_mobilenet_test20.h5",  # path to save the best model
        monitor="val_accuracy",
        save_best_only=True,  # only save the best model based on `monitor`
        verbose=1,  # print a message whenever a new best model is saved
        mode="max",  # since we're monitoring accuracy, we want to maximize it
...
mobilenet_test20_train_accuracy = history.history["accuracy"]
mobilenet_test20_val_accuracy = history.history["val_accuracy"]
mobilenet_test20_train_loss = history.history["loss"]
mobilenet_test20_val_loss = history.history["val_loss"]

# load the saved best model
best_model_0 = tf.keras.models.load_model(
    "save_best_models/best_model_mobilenet_test20.h5"
)

# evaluate the test data using the loaded best model
mobilenet_test20_test_loss, mobilenet_test20_test_accuracy = best_model_0.evaluate(
    test_data
)

end_time = time()
execution_time_test20 = end_time - start_time

logging.info("mobilenet test20 was done")
# --------------------------------------------------------------------------------------- #

# _______ RECORD ALL RESULTS _______ #
metrics = {
    "mobilenet_test19": {
        "train_accuracy": mobilenet_test19_train_accuracy,
        "val_accuracy": mobilenet_test19_val_accuracy,
        "train_loss": mobilenet_test19_train_loss,
        "val_loss": mobilenet_test19_val_loss,
        "test_accuracy": mobilenet_test19_test_accuracy,
        "test_loss": mobilenet_test19_test_loss,
        "execution_time": execution_time_test19,
    },
    "mobilenet_test20": {
        "train_accuracy": mobilenet_test20_train_accuracy,
        "val_accuracy": mobilenet_test20_val_accuracy,
        "train_loss": mobilenet_test20_train_loss,
        "val_loss": mobilenet_test20_val_loss,
        "test_accuracy": mobilenet_test20_test_accuracy,
        "test_loss": mobilenet_test20_test_loss,
        "execution_time": execution_time_test20,
    },
}


# ________________ SAVE MODEL PERFORMANCE LOG TO .JSON ________________ #
test_name = "mobilenet_finetune9"
