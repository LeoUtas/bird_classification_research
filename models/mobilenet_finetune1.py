...
# _______ MOBILENET TEST1: NO DATA AUGMENTATION & NO RE-TRAIN LAYER & USE CALLBACKS _______ #
factor = 0.7
patience = 5
...
callbacks = [
    ...
    tf.keras.callbacks.ModelCheckpoint(
        "save_best_models/best_model_mobilenet_test1.h5",  # path to save the best model
...
best_model_0 = tf.keras.models.load_model(
    "save_best_models/best_model_mobilenet_test1.h5"
)

# evaluate the test data using the loaded best model
mobilenet_test1_test_loss, mobilenet_test1_test_accuracy = best_model_0.evaluate(
    test_data
)
...
callbacks = [
    ...
    tf.keras.callbacks.ModelCheckpoint(
        "save_best_models/best_model_mobilenet_test4.h5",  # path to save the best model
        monitor="val_accuracy",
        save_best_only=True,  # only save the best model based on `monitor`
        verbose=1,  # print a message whenever a new best model is saved
        mode="max",  # since we're monitoring accuracy, we want to maximize it
...
mobilenet_test4_train_accuracy = history.history["accuracy"]
mobilenet_test4_val_accuracy = history.history["val_accuracy"]
mobilenet_test4_train_loss = history.history["loss"]
mobilenet_test4_val_loss = history.history["val_loss"]

# load the saved best model
best_model_0 = tf.keras.models.load_model(
    "save_best_models/best_model_mobilenet_test4.h5"
)

# evaluate the test data using the loaded best model
mobilenet_test4_test_loss, mobilenet_test4_test_accuracy = best_model_0.evaluate(
    test_data
)
...
metrics = {
    ...
    "MobileNet_test4": {
        "train_accuracy": mobilenet_test4_train_accuracy,
        "val_accuracy": mobilenet_test4_val_accuracy,
        "train_loss": mobilenet_test4_train_loss,
        "val_loss": mobilenet_test4_val_loss,
        "test_accuracy": mobilenet_test4_test_accuracy,
        "test_loss": mobilenet_test4_test_loss,
        "execution_time": execution_time_test4,
    },
}

# ________________ SAVE MODEL PERFORMANCE LOG TO .JSON ________________ #
test_name = "mobilenet_finetune1"
